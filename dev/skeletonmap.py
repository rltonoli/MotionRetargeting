        ...
        for joint in animation.getlistofjoints():
            if isroot(joint.name,root):
                ...
                if iships(joint.name, root):
                    self.hips = joint
            #Spine
            elif iships(joint.name, hips):
                self.hips = joint
            elif isspine3(joint.name, spine3):
                self.spine3 = joint
            elif isspine2(joint.name, spine2):
                self.spine2 = joint
            elif isspine1(joint.name, spine1):
                self.spine1 = joint
...
def getmatchingjoint(jointname, animation):
    ...
    custommap = [joint.name if joint else None for joint in animation.getskeletonmap().alljoints]
