...
class Surface:
    ...
    def NormalizeSurfaceData(self, hipsjoint):
        ...
        ratio = hipsjoint.position[0][1]/hipsBasePosition[1]
        #TODO: Eu esqueci pq faço isso, investigar e documentar pra não esquecer de novo neh
        transX = hipsjoint.position[0][0] - hipsBasePosition[0]*ratio
        transZ = hipsjoint.position[0][2] - hipsBasePosition[2]*ratio
        for point in self.points:
    ...
    def getCentroid(self, meshtype, index, frame):
        if meshtype=='body':
            mesh = self.bodymesh[index]
        elif meshtype=='head':
            mesh = self.headmesh[index]
        centroid = np.mean([mesh[0].position[frame], mesh[1].position[frame], mesh[2].position[frame]], axis=0)
        return centroid[:-1]
...
def FindHandsZeroSpeedGaps(animation, threshold=None, plot=False, minrangewide = 20, minpeakwide=40):
    ...
    rightHandPos = np.asarray([rightHand.position[j,:] for j in range(rightHand.position.shape[0])]).T
    leftHandPos = np.asarray([leftHand.position[j,:] for j in range(leftHand.position.shape[0])]).T
...
#        plt.plot(leftHandVel[:], label='Left Hand')
    ...
    for i in range(rightHandPositions.shape[0]):
        ...
        rightHandPositions[i,:] = np.mean(rightHand.position[center-window:center+window],axis=0)
...
#        plt.plot(rightHandVel[:], label='Right Hand')
    ...
    for i in range(leftHandPositions.shape[0]):
        ...
        leftHandPositions[i,:] = np.mean(leftHand.position[center-window:center+window],axis=0)
...
def GetCalibrationFromBVHS(frontAnimName, headAnimName, backAnimName, savefile=True, debugmode = False, minrangewide = 20, minpeakwide=40):
    """
...
#    arq = os.path.join(realpath, 'Superficie\Frente_2_mcp.bvh')
    ...
    animation = pyanimation.ReadFile(arq)
    print('Getting first surface data')
    right, left, rightrange, leftrange = FindHandsZeroSpeedGaps(animation, plot = True, minrangewide = minrangewide, minpeakwide=minpeakwide)
    threshold = 0.9
    while len(right) < 14 and len(left) < 14:
        right, left, rightrange, leftrange = FindHandsZeroSpeedGaps(animation, threshold, plot = True, minrangewide = minrangewide, minpeakwide=minpeakwide)
