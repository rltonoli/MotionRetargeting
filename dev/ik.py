...
#class Jacobian:
#    
#    def __init__(self, avatarAnimation, avatarSurface, mocapAnimation, mocapSurface, egocoordinates):
#        self.avatar = avatarAnimation
#        self.surface_avatar = avatarSurface
#        self.mocap = mocapAnimation
#        self.surface_mocap = mocapSurface
#        self.ego = egocoordinates
#        nEndEffector = 4 #Hands and Feet
#        nJoints = len(self.mocap.root.printHierarchy([]))
#        self.jEndEffector = np.zeros([3*nEndEffector, nJoints])
#        self.jTargets = np.zeros([3*nEndEffector, nJoints])
#        self.dDistance = np.empty(3*nEndEffector) #Targets Positions - EndEffector
#        self.dTheta = np.empty(nJoints) #Changes in joint angles
#        self.dPreTheta = np.empty(nJoints)
#        
#        self.dT = np.empty(3*nEndEffector) # Linearized change in end effector positions based on dTheta
#        
#        self.dSclamp = np.empty(nEndEffector)
#        self.orArray = np.empty(nEndEffector)
#        self.Jnorms = np.empty([nEndEffector, nJoints])
#        
#        self.Reset()
#        
#        
#    def Reset(self, DefaultDampingLambda = 1.1):
#        
#        self.DampingLambda = DefaultDampingLambda
#        self.DampingLambdaSq = np.square(self.DampingLambda)
#        self.dSclamp[:] = np.inf
#
#
#    # Compute the deltaS vector, dS, (the error in end effector positions
#    # Compute the J and K matrices (the Jacobians)
#    def ComputeJacobian(self, frame):
#        #For each endeffector
#        for ego in self.egocoordinates:
#            #Compute the delta Distance value (differences from end effectors to target positions).
#            self.dDistance = - (ego.target - self.ego.joint.position[frame])
...
class SimpleJacobian:
    ...
    def jacobianTranspose(self, frame, target, rotnextframe = False):
        """
